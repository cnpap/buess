...
function HeaderTab({ routeIframe, to, close }: HeaderTabProps) {
  ...
  return (
    <div className={'relative '}>
      <div
        className={cn(
          'select-none h-10 border rounded-md pl-4 pr-8 py-2 flex items-center gap-1 bg-white dark:bg-gray-800 transition duration-50 ease-in-out transform hover:cursor-pointer hover:bg-white dark:hover:bg-gray-900',
      ...
      {!routeIframe.src.endsWith('/home') && (
        ...
        >
          <X className="transition duration-300 delay-100 w-5 h-5 text-slate-800 dark:text-gray-200 hover:text-white transform cursor-pointer" />
        </div>
      )}
    </div>
  );
